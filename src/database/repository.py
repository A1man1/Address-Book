...
from src.database.dbconfig import db
...
class	CreateTimeModelMixin(BaseModel):
    """
    Model Mixin for created and updated timestamp information tables.
    """
...
def for_pool(rec):
    data = []
    if rec:
        data = list(map(lambda x: inter(x), rec))
    return data
...
class BaseRepository(abc.ABC):
    ...
    async def _fetch_by_id(self, id: int) -> Mapping:
        ...
        if not rec:
            raise f"Not Found: { self._table.name.capitalize() } Object with ID { id }"
        return rec
    ...
    async def list(self, **kwargs):
        ...
        return [self._schema_out(**dict(zip([*row.keys()], row))) for row in rows]

    async def fetch_by_id(self, id: int) -> BaseSchema:
        record = await self._fetch_by_id(id)
        data = self._schema_out(**dict(zip(record.keys(), record.values())))
        return data
